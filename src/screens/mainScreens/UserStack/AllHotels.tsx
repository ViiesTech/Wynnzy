  ...
  const renderHotels = ({ item, index }) => {
    return (
      <TouchableOpacity onPress={() => navigation.navigate('StoreDetails', { _id: item._id, managerId: item.managerId })} key={index} style={{ padding: 10, width: responsiveWidth(48) }}>
        ...
        <BoldText title={item.businessName} mrgnTop={10} fontSize={responsiveFontSize(2.4)} color="#2A1E51" />
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1, backgroundColor: Colors.white, padding: responsiveHeight(2) }}>
      <View style={{ flexDirection: 'row', paddingVertical: responsiveHeight(1), alignItems: 'center', justifyContent: 'space-between' }}>
        <BackIcon />
        <NormalText color={Colors.themeText2} alignSelf="center" fontWeight="900" fontSize={responsiveFontSize(2.7)} title="Hotels/DayCares" />
        ...
        <View style={{ flex: 0.9 }}>
          {data?.length > 0 ? (
            <FlatList
              ...
              numColumns={2}
              contentContainerStyle={{ justifyContent: 'space-between', gap: responsiveHeight(1) }}
              data={data}
              renderItem={renderHotels}
              keyExtractor={(item) => item._id.toString()} // Unique key for each item
            />
          ) : (
            <NormalText fontSize={responsiveFontSize(2.5)} mrgnTop={responsiveHeight(10)} alignSelf="center" txtAlign="center" title="No Business Stores Found" />
          )}
