...
import { getAllBookingsByUserId, getServicesByCategory } from '../../../GlobalFunctions';
...
const ViewBookings = ({ navigation, route }) => {
  const [currentCategory, setCurrentCategory] = useState('Pending');
  const [bookings, setBookings] = useState([]);
  const { _id, type } = useSelector(state => state.user.userData);
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <FilterCard
              ...
              handlePress={() =>
                navigation.navigate('ViewBookedServices', { type: 'user', service: item.serviceId, bookingId: item?._id, bookingStatus: item?.status })}
            />
          )}
        />
      ) : (
